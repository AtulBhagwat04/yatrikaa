...
const seedBeaches = async () => {
  try {
    ...
    for (const beach of beachesData) {
      ...
      const existing = await Place.findOne({ place_id: beach.place_id });
      if (existing) {
        console.log(`- Skipping ${beach.name} (Already exists in DB)`);
        continue;
      }
      ...
      for (let i = 0; i < beach.images.length; i++) {
        ...
        try {
          ...
          // Fallback to original URL if upload fails (though Unsplash hotlinking is unreliable)
      ...
      // Map to Place model (handling types vs category)
      const placeToSave = new Place({
        ...beach,
        types: [beach.category.toLowerCase()],
      ...
      console.log(`- Saved ${beach.name} to Database.`);
    }
